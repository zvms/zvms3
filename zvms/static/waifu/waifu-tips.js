...
// 后端接口
live2d_settings['modelAPI'] = '//172.31.2.4:11451/';   // 自建 API 修改这里
...
//看板娘样式设置
live2d_settings['waifuSize'] = '280x500';    // 看板娘大小，例如 '280x250', '600x535'
...
live2d_settings['waifuEdgeSide'] = 'bottom:50';     // 看板娘贴边方向，例如 'left:0'(靠左 0px), 'right:30'(靠右 30px)
...
function initModel(waifuPath, type) {
    ...
    $(".waifu").css(live2d_settings.waifuEdgeSide[0], live2d_settings.waifuEdgeSide[1] + 'px');
    ...
    $('.waifu-tool .fui-info-circle').click(function () {
        ...
        const match = $('textarea#content').val().match(/modelid:(?<id>\d+)/);
        if (match !== null) {
            loadModel(match.groups['id']);
        }
    });
...
function loadTipsMessage(result) {
    ...
    $('.waifu-tool .fui-cross').click(function () {
        ...
        window.setTimeout(function () { $('.waifu').remove(); }, 1300);
    });
